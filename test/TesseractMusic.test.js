...
describe("TesseractMusic", function () {
    ...
    describe("Deployment", function () {
        it("Should track name, symbol, URI, royalty fee and artist", async function () {
            ...
            expect(await tesseractmusic.name()).to.equal(nftName);
            expect(await tesseractmusic.symbol()).to.equal(nftSymbol);
            expect(await tesseractmusic.baseURI()).to.equal(URI);
            expect(await tesseractmusic.royaltyFee()).to.equal(royaltyFee);
            expect(await tesseractmusic.artist()).to.equal(artist.address);
        });


        it("Should mint then list all the music nfts", async function () {
            expect(await tesseractmusic.balanceOf(tesseractmusic.address)).to.equal(8);
            ...
            await Promise.all(prices.map(async (i, indx) => {
                const item = await tesseractmusic.marketItems(indx)
        ...
        it("Ether balance should equal deployment fees", async function () {
            expect(await ethers.provider.getBalance(tesseractmusic.address)).to.equal(deploymentFees)
        });
    });
...
// 3 cases passing
    ...
    describe("Updating royalty fee", function () {
        ...
        it("Only deployer should be able to update royalty fee", async function () {
          ...
          await tesseractmusic.updateRoyaltyFee(fee)
          await expect(
            tesseractmusic.connect(user1).updateRoyaltyFee(fee)
          ).to.be.revertedWith("Ownable: caller is not the owner");
          expect(await tesseractmusic.royaltyFee()).to.equal(fee)
        });
      ...
      describe("Buying tokens", function () {
        it("Should update seller to zero address, transfer NFT, pay seller, pay royalty to artist and emit a MarketItemBought event", async function () {
          ...
          await expect(tesseractmusic.connect(user1).buyToken(0, { value: prices[0] }))
            .to.emit(tesseractmusic, "MarketItemBought")
            .withArgs(
              0,
              deployer.address,
              user1.address,
          ...
          expect((await tesseractmusic.marketItems(0)).seller).to.equal("0x0000000000000000000000000000000000000000")
          ...
          expect(await tesseractmusic.ownerOf(0)).to.equal(user1.address);
        })
        it("Should fail when ether amount sent with transaction does not equal asking price", async function () {
          ...
          await expect(
            tesseractmusic.connect(user1).buyToken(0, { value: prices[1] })
      ...
      describe("Reselling tokens", function () {
        beforeEach(async function () {
          ...
          await tesseractmusic.connect(user1).buyToken(0, { value: prices[0] })
        })
        ...
        it("Should track resale item, incr. ether bal by royalty fee, transfer NFT to marketplace and emit MarketItemRelisted event", async function () {
          ...
          const initMarketBal = await ethers.provider.getBalance(tesseractmusic.address)
          // user1 lists the nft for a price of 2 hoping to flip it and double their money
          await expect(tesseractmusic.connect(user1).resellToken(0, resaleprice, { value: royaltyFee }))
            .to.emit(tesseractmusic, "MarketItemRelisted")
            .withArgs(
              0,
              user1.address,
              resaleprice
            )
          const finalMarketBal = await ethers.provider.getBalance(tesseractmusic.address)
          ...
          expect(await tesseractmusic.ownerOf(0)).to.equal(tesseractmusic.address);
          // Get item from items mapping then check fields to ensure they are correct
          const item = await tesseractmusic.marketItems(0)
        ...
        it("Should fail if price is set to zero and royalty fee is not paid", async function () {
          await expect(
            tesseractmusic.connect(user1).resellToken(0, 0, { value: royaltyFee })
          ).to.be.revertedWith("Price must be greater than zero");
          await expect(
            tesseractmusic.connect(user1).resellToken(0, toWei(1), { value: 0 })
      ...
      describe("Getter functions", function () {
        ...
        beforeEach(async function () {
          ...
          await (await tesseractmusic.connect(user1).buyToken(0, { value: prices[0] })).wait();
          // user1 purchases item 1.
          await (await tesseractmusic.connect(user1).buyToken(1, { value: prices[1] })).wait();
          // user2 purchases item 4.
          await (await tesseractmusic.connect(user2).buyToken(4, { value: prices[4] })).wait();
        })
    
        it("getAllUnsoldTokens should fetch all the marketplace items up for sale", async function () {
          const unsoldItems = await tesseractmusic.getAllUnsoldTokens()
        ...
        it("getMyTokens should fetch all tokens the user owns", async function () {
          ...
          let myItems = await tesseractmusic.connect(user1).getMyTokens()
          ...
          myItems = await tesseractmusic.connect(user2).getMyTokens()
          // Check that the returned my items array is correct
